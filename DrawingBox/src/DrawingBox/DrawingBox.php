...
class DrawingBox extends PluginBase {
  ...
  public function BoxSettingGUI($player) {
    ...
    $inv = LifeInventoryLib::getInstance ()->create("DOUBLE_CHEST", new Position($playerPos->x, $playerPos->y - 2, $playerPos->z, $playerPos->getWorld()), "[ 뽑기상자 ] | 상자세팅",$player);
  ...
  public function BoxSeeSettingGUI($player) {
    $name = $player->getName ();
    $playerPos = $player->getPosition();
    $inv = LifeInventoryLib::getInstance ()->create("DOUBLE_CHEST", new Position($playerPos->x, $playerPos->y - 2, $playerPos->z, $playerPos->getWorld()), "[ 뽑기상자 ] | 상자세팅정보",$player);
    $count = 0;
    $boxname = $this->pldb [strtolower($name)] ["BoxName"];
    foreach($this->boxdb [$boxname] as $count => $v) {
      if ($count != "count"){
